...
public class TileLayerEditor_SceneView
{
	...
	{
		Handles.BeginGUI();
		GUI.Box(new Rect (0, Screen.height - 218, 100, 198), "");
		GUILayout.BeginArea(new Rect(5, Screen.height-208, 85,188));
		...
		if(selection.selectedTilesList.Length == 1 && selection.selectedTilesList[0].id != -1) {
			GUILayout.Label(string.Format("{0} x {1} ({2})", selection.selectedTilesWidth, selection.selectedTilesWidth == 0 ? 0 : selection.selectedTilesList.Length/selection.selectedTilesWidth, selection.selectedTilesList[0].id), "button", GUILayout.ExpandWidth(true));	
		} else {
			GUILayout.Label(string.Format("{0} x {1}", selection.selectedTilesWidth, selection.selectedTilesWidth == 0 ? 0 : selection.selectedTilesList.Length/selection.selectedTilesWidth), "button", GUILayout.ExpandWidth(true));	
		}
