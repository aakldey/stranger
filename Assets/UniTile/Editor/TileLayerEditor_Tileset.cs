...
public class TileLayerEditor_Tileset
{
	...
	private bool? 		_useScrollView 	= null;
	private bool 		useScrollView 	{
		get {
			if(_useScrollView == null) {
				_useScrollView = EditorPrefs.GetBool("unitile_use_scrollview", false);
			}
			
			return (bool)_useScrollView;
		}
		set {
			if(value != _useScrollView) {
				_useScrollView = value;
				EditorPrefs.SetBool("unitile_use_scrollview", value);
			}
	...
	public void OnInspectorGUI(TileLayer layer, UniTileTemplate selection, TileLayerEditor editor)
	{
		...
		float w = Mathf.Min(Screen.width - 25, layer.material.mainTexture.width); //Mathf.Min(rect.width,layer.material.mainTexture.width);
		...
		if(useScrollView) {
			...
			scrollPos = EditorGUILayout.BeginScrollView(scrollPos, false, false, GUILayout.Height(h));
