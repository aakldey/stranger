...
function MoveToCenterOfGate () {
	var startPos = transform.position;
	// Convert transform.position to board position and back to world position to get
	// center of tile in world coordinates
	var boardPos = Tile.GetMapPosition (transform.position, 2);
	var endPos = Tile.GetWorldPosition (boardPos, 2);
	var t = 0.0;
	while (t <= 1.0) {
		t += Time.deltaTime * 3;
...
function FlareEffect () {
	...
	for (flareObject in flareObjects) {
		flareObject.renderer.sortingLayerID = 2;
	}
