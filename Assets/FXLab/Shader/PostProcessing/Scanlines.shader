	...
	SubShader {
		...
		Pass {
			...
			float4 frag( v2f o ) : COLOR
			{
				...
				fixed3 lineColor = tex2D(_LineMap, o.uv + uv_offset).xyz;
				lineColor.rgb = clamp(lineColor.rgb*_LineIllumination, float3(0, 0, 0), float3(1, 1, 1));
				return fixed4(lerp(fixed3(1, 1, 1), lineColor.rgb, _LineStrength), 1);
			}
			ENDCG
        }
    }
